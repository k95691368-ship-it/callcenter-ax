...
콜센터 AX(AI Transformation) 워크벤치.

콜센터 현장의 실제 업무 흐름 — *"STT, LLM, NLP, RAG를 활용한 녹취 기반 분류·요약·검색,
VOC·의도 분석, 상담 품질 평가 자동화(Auto QA)"* — 를 문서가 아니라 **동작하는 파이프라인**
그대로 구현했다.
...
| [/about](https://callcenter-ax.pages.dev/about) | — | 기능 ↔ 구현 매핑표 · 구조도 · 라이브/데모 정직 구분표 · **실측 운영 지표**(D1 텔레메트리 집계: 호출 수·라이브 비율·평균 지연) |
...
Claude 호출만 **토큰 단가로 합산하는 롤링 24시간 금액 상한**($3, `budget.js`)이 있어
소진 시 서비스 중단 없이 오픈소스로 자연 강등된다 — 과금 폭주가 구조적으로 불가능.
회수 상한이 아니라 금액인 이유는, 전사 8,000자를 넣는 통화 분석과 집계 수치만 넣는 VOC
리포트의 실제 지출이 몇 배 차이라 "N회 안에서는 안전하다"가 성립하지 않기 때문이다.
이와 별개로 전 엔드포인트가 공유하는 일일 호출 캡(300회)이 남용을 막는다 — 둘은 다른
장치이고, 화면에는 각각의 남은 양이 따로 표시된다.
프롬프트 캐싱 실측: 동일 요청 입력 토큰 1,211 → 299 (-75%).
...
- [개선기획안.md](개선기획안.md) — 배포 후 개선 사이클 41회 기록 (진단 → 구현 → 실측)
