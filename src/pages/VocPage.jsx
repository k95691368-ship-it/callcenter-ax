...
export default function VocPage() {
  ...
  const agg = useMemo(() => {
    ...
    const avgMin = calls.reduce((s, c) => s + c.minutes, 0) / calls.length
  ...
  return (
    <div className="tool-page">
      <header className="tool-header">
        ...
        <h1>VOC 대시보드</h1>
        <p>
          통화 분석 결과가 쌓이면 이런 그림이 됩니다 — 가상 통화 <strong>10건</strong>의 분석
          라벨을 유형·감정·일별로 집계했습니다. 어떤 문의가 몰리는지, 강성 민원이 어디서
          나오는지가 한눈에 보입니다. (1차 배포는 내장 샘플 기준, 실시간 누적은 개선 사이클 예정)
        </p>
      </header>
