...
export const HOLDOUT_SET = [
  ...
  // 무관하지만 **재작성 규칙의 행위어를 포함한** 질의.
  //
  // 이 줄들은 다른 줄과 성격이 다르다. 위 무관 질의(코스피·맛집·출시일)는 규칙에 아예
  // 걸리지 않아, 재작성 층이 무관 질의를 어떻게 다루는지 **한 번도 재지 못했다**.
  // 그 결과 abstainAccuracy 1.000이 나왔지만 실제로는 '바꾸/묶/그만두'가 든 도메인 밖
  // 질의 12건 중 8건이 '근거 없음 → 확신'으로 뒤집히고 있었다(실측).
  // 재지 않는 것은 지켜지지 않는다 — 그래서 그 경로를 셋에 넣는다.
  //
  // 이 질의들은 결함을 발견한 뒤 역으로 쓴 것이라 '보지 않은 질의'가 아니다.
  // 회귀 방지용 고정 케이스로만 쓰고, 재작성 개선의 근거로는 삼지 않는다.
  { q: '머리 스타일 바꾸고 싶어요', expected: null, kind: '무관·행위어' },
  { q: '학원 그만두려면 어떻게', expected: null, kind: '무관·행위어' },
  { q: '짐 좀 묶어서 보내주세요', expected: null, kind: '무관·행위어' },
]
