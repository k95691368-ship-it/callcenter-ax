...
export const PII_RULES = [
  {
    type: 'rrn',
    label: '주민등록번호',
    // 생년월일 6자리 + 성별코드 + 6자리. 월·일이 말이 되는 값일 때만.
    //
    // 성별코드는 1~4가 내국인, **5~8이 외국인등록번호**다. 1~4만 보고 있어서
    // 외국인등록번호는 한 글자도 가려지지 않았고, total도 0이라 화면에 "가렸습니다"
    // 경고조차 뜨지 않았다 — 가리지 못했다는 사실 자체가 보이지 않는 상태였다.
    // 9·0은 1900년 이전 출생자용이라 실무에서 거의 없지만 빠뜨릴 이유도 없어 함께 넣는다.
    re: /\b(\d{2})(0[1-9]|1[0-2])(0[1-9]|[12]\d|3[01])[-\s]?(\d)\d{6}\b/g,
